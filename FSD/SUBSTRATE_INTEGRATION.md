...
|---|---|---|---|
| **Now (v0.1.0-dev)** | v0.6.0 + v0.6.1 published (pipeline + SecretsService); Appendix A contract pinned, typed-writes pending v0.6.3 / v0.7.0. | v0.1.2 ships all 8 federation-consensus `MessageType` variants (CIRISEdge#6 closed). | All 8 wire types published with `impl Message`; `NodeCore` service struct + 8 typed handlers + `MockEngine` in `tests/support/`. 20 tests green (13 unit + 7 integration). |
