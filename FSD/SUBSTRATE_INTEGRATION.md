...
|---|---|---|---|
| **Now (v0.1.0-dev)** | v0.6.0 published; Appendix A contract pinned. | v0.1.1 latest tag; MessageType expansion (CIRISEdge#6) open. | Skeleton committed — wire types + trait surface. `cargo check` clean; 6 unit tests pass. |
...
| **OQ-3: in-memory mock engine.** Where does the `MockNodeCoreEngine` live for tests — node-core's `tests/support/` or a separate `ciris-node-core-test` crate? | v0.1.0 cut-time | `tests/support/` in this crate; promote to a separate crate only when downstream consumers (safety.ciris.ai backend tests) want it. |
