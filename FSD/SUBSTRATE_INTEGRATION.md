...
|---|---|---|---|
| **Now (v0.1.0-dev)** | **v1.5.1 pinned**. Trust substrate flipped to signed-grant projection: `ciris_persist::federation::trust_grant::{TrustPurpose, TrustGrantRow, TrustGrantFilter}` + `ciris_persist::audit::AuditService` (with `lookup_trust_grant` + `list_trust_grants` over the `federation_trust_grants` table). Old `FederationDirectory.lookup_trust` / `list_trusted_keys` paths still compile (back-compat); v1.6.0 drops the V020 trust columns. **CIRISPersist#53 closed.** | v0.1.4 pinned. Surface stable since v0.1.2. | **Step 10 of FSD §8 landed.** `crate::trust::resolve_trust` and `crate::routing::route_deferral` now consume `&impl AuditService` instead of `&impl FederationDirectory`. `TrustEdge::Registry` variant dropped (the v1.3.0 peer-vs-registry distinction collapsed in the signed-grant model — Direct + Transitive + Untrusted remain). MockEngine impls `AuditService` (3 required method stubs + 2 real trust-grant methods + 12 trait defaults); `set_trust_grant` fixture replaces `grant_trust` for tests. New `audit_err` maps `audit::Error` → `SubstrateError`. 110 tests green (67 unit + 5 aggregate + 7 handlers + 11 routing + 9 substrate + 11 trust). |
...
| **OQ-7: persist v0.7.0 collapse.** Node-core currently maintains parallel wire types alongside persist α3's `cirisnode::types::*`. When do we collapse, and how? | persist v0.7.0 release | **CLOSED 2026-05-13** — collapse landed against persist v0.7.4 in one focused commit. All 5 steps executed: (1) `crate::{cell, signature, witness, contribution, vote, ledger, engine, error, identity}` modules deleted (~600 LOC); persist's types re-exported via `crate::substrate`. (2) `crate::payloads::*` structs renamed with `Payload` suffix; envelope-level fields (id, author, cell, signature, timestamps) moved to persist's envelope, only policy fields remain. (3) `NodeCore<E: NodeCoreService>` generic — drops `Arc<dyn>`. (4) `MockEngine` rewritten to impl `NodeCoreService` directly with RPITIT — no `async_trait` on the engine surface (still used for edge's `Handler<M>` trait, which keeps async_trait). (5) `crate::wire` has 8 `#[serde(transparent)]` newtypes (`ContributionSubmit`, `VoteCast`, `DeferralRequest`, `DeferralResponse`, `ExpertiseAttestationPublish`, `ModerationEventPublish`, `SlashingAttestationPublish`, `ReconsiderationRequest`), each with `impl Message`. 26/26 tests green. |
