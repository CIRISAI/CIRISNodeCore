...
|---|---|---|---|
| **Now (v0.1.0-dev)** | **v1.3.0 pinned**. Trust-axis primitive absorbed: `ciris_persist::federation::FederationDirectory` trait (4 trust methods + 14 directory methods) + `TrustType` / `TrustRelationship` / `TrustGrant` / `TrustRow` / `TrustFilter` types. NodeCoreService surface unchanged (15 methods). Still pending in CIRISPersist#47: `federation_keys.consent_role` / `edge_detection_events` / `cirislens_audit_writer` — all gated on CIRISAgent#760 §RC consent_role lock. | v0.1.4 pinned. Surface stable since v0.1.2. | **M1 closed from node-core's side.** `crate::trust` `pub use`'s persist's types directly; `crate::trust::resolve_trust` + `crate::routing::route_deferral` compose the directory with NodeCoreService. `fed_err` maps `federation::Error` → `SubstrateError`. MockEngine impls persist's full FederationDirectory (4 real trust methods + 14 stubs for public-key/attestation/revocation/PQC surfaces). 62 tests green (19 unit + 5 aggregate + 7 handlers + 11 routing + 9 substrate + 11 trust). |
