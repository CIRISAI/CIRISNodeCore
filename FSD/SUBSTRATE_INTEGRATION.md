...
|---|---|---|---|
| **Now (v0.1.0-dev)** | v0.7.0-α3 (rev `5f884a4`) ships `cirisnode::types::*` wire types + `NodeCoreService` trait (RPITIT, 8 typed-writes + 5 read clusters). α4-α6 pending: PostgresBackend impl, Engine PyO3 wraps, release tag. | v0.1.2 ships all 8 federation-consensus `MessageType` variants (CIRISEdge#6 closed). | Pinned against persist α3 via `crate::substrate` re-export module. Local wire types (`crate::cell`, `crate::vote`, etc.) + `NodeCoreEngine` trait + `MockEngine` kept in parallel during the v0.1.0 collapse window — divergence inventory in §6 OQ-7. 20 tests green (13 unit + 7 integration). |
