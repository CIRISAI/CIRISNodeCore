...
|---|---|---|---|
| **Now (v0.1.0-dev)** | **v0.9.1 pinned** (lockstep with `ciris-lens-core`). `src/cirisnode/` track API-stable since v0.7.1 — v0.8.x / v0.9.x churn was CIRISAgent 2.9.0 adoption (cirisgraph, audit, telemetry, incident records absorption), not relevant to node-core. Trait surface: 15 methods (9 typed-writes + 6 reads, RPITIT). **Zero open substrate asks.** | **v0.1.4 pinned** (lockstep with `ciris-lens-core`). Surface stable since v0.1.2; v0.1.3 / v0.1.4 were persist + verify lockstep bumps. | OQ-7 collapse landed. `crate::sign` (envelope builders with consumer-supplied `EnvelopeSigner`), `crate::aggregate` (§7 weighted aggregate with fail-secure `BelowQuorum` variant), `crate::routing` (§3.3 diversity selection with `ContributorMetadataProvider`). 43 tests green (15 unit + 5 aggregate + 7 handlers + 7 routing + 9 substrate contract). |
