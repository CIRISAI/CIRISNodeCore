...
|---|---|---|---|
| **Now (v0.1.0-dev)** | **v0.7.4 shipped** (tag + PyPI). v0.7.2 closed CIRISPersist#32 with `put_promotion_attestation`. Trait surface: 15 methods (9 typed-writes + 6 reads, RPITIT). **Zero open substrate asks.** | v0.1.2 ships all 8 federation-consensus `MessageType` variants (CIRISEdge#6 closed). | **OQ-7 collapse landed.** Persist's types are canonical; node-core publishes typed `*Payload` policy schemas + 8 `#[serde(transparent)]` wire newtypes + `NodeCore<E: NodeCoreService>` generic + RPITIT `MockEngine` in `tests/support/`. Parallel local types removed (`src/{cell, signature, witness, contribution, vote, ledger, engine, error, identity}.rs` deleted). 26 tests green (10 unit + 7 handlers + 9 substrate contract). |
