...
|---|---|---|---|
| **Now (v0.1.0-dev)** | **v0.7.4 shipped** (tag + PyPI). v0.7.2 closed CIRISPersist#32 (canonical-promotion gap) with Option B: `put_promotion_attestation` + `PromotionAttestation` + `TargetRowKind` types — transactional flip with affected-row-count assertion. Trait grew 14 → 15 methods. v0.7.3 was CI hygiene only. v0.7.4 wired lens-side pipeline orchestration (transparent to node-core). **Zero open substrate asks.** | v0.1.2 ships all 8 federation-consensus `MessageType` variants (CIRISEdge#6 closed). | Pinned against persist `tag = "v0.7.4"`. `crate::substrate` re-exports refreshed (added `PromotionAttestation`, `TargetRowKind`). `tests/substrate_contract.rs` (9 tests) round-trips every wire type + every trait method including the new promotion path. Local parallel wire types still present pending OQ-7 collapse commit. 30 tests green (14 unit + 7 handlers + 9 substrate contract). |
